function ghm(θ,x,y)
  return(θ[1] + θ[2] * x + θ[3] * x^3 + θ[4] * y)
end
...
function genData(n, Θ, x0, x00, p1, lam1, lam2, seed)
  ...
  x[1] = ghm(Θ,x0,x00) + noiseMix(p1, lam1, lam2)
  x[2] = ghm(Θ,x[1],x0) + noiseMix(p1, lam1, lam2)
  for i in 3:1:n
    x[i] = ghm(Θ,x[i-1],x[i-2]) + noiseMix(p1, lam1, lam2)
  end
...
function genDataf1(n, Θ, x0, x00, lam, seed)
  ...
  x[1] = ghm(Θ,x0,x00) + noiseMixf1(lam)
  x[2] = ghm(Θ,x[1],x0) + noiseMixf1(lam)
  for i in 3:1:n
    x[i] = ghm(Θ,x[i-1],x[i-2]) + noiseMixf1(lam)
  end
